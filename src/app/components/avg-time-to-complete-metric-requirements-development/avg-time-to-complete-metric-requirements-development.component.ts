...
@Component({
  selector: 'app-avg-time-to-complete-metric-requirements-development',
  templateUrl: './avg-time-to-complete-metric-market-requirements-development.html'
})
export class AverageTimeToCompletionRequirementsDevelopmentBarChartComponent {
  @HostBinding() class = 'data_table fixedMetrics m-0';
  @HostBinding() style = {display: 'block'};
  averageTimeToCompleteMetricArr: any[] = [];
  activityMapObj: any = {};
  colorSchemeComp = ['#4473C5', '#4473C5', '#4473C5'];
  xAxisLabel = 'Requirements Development';

  constructor(private restAPIService: RESTAPIService, private router: Router) {
    this.restAPIService.getActivities(`WorkCategory eq 'Requirements Development' and Status eq 'Completed'`).subscribe((response: any) => {
