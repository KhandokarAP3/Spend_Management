...
@Component({
  selector: 'app-requirementsdevelopment',
  templateUrl: './requirementsdevelopment.component.html'
})
export class RequirementsDevelopmentComponent extends PageComponentParent {
  ...
  constructor(private router: Router, private restAPIService: RESTAPIService, public commonService: CommonService) {
    super();
    // this.restAPIService.getRequirementsDevelopmentData().subscribe((response: any) => {
    ...
    let filter = `substringof('${AppConstants.REQ_DEV}', WorkCategory)`;

    forkJoin([this.restAPIService.getRequirementsDevelopmentData(), this.restAPIService.getFilteredTemplatesPageData(filter), this.restAPIService.getTutorialsPageData(filter)]).subscribe((forkJoinResponses: any) => {
