...
export class MarketResearchComponent extends PageComponentParent {
  ...
  constructor(private router: Router, private restAPIService: RESTAPIService, public commonService: CommonService) {
    super();
    // this.restAPIService.getMarketResearchData().subscribe((response: any) => {
    ...
    let filter = `substringof('${AppConstants.MARKET_RESEARCH}', WorkCategory)`;

    forkJoin([this.restAPIService.getMarketResearchData(), this.restAPIService.getFilteredTemplatesPageData(filter), this.restAPIService.getTutorialsPageData(filter)]).subscribe((forkJoinResponses: any) => {
      if (forkJoinResponses[0].data.length) {
        ...
        console.log('Either no requirements development work category data was found in the SP list or the system experienced an error.');
      }
