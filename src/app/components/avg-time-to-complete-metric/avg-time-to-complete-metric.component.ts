...
@Component({
  selector: 'app-avg-time-to-complete-metric',
  templateUrl: './avg-time-to-complete-metric.component.html'
})
export class AverageTimeToCompletionBarChartComponent {
  @HostBinding() class = 'data_table fixedMetrics m-0';
  @HostBinding() style = {display: 'block'};
  averageTimeToCompleteMetricArr: any[] = [];
  activityMapObj: any = {};
  colorSchemeComp = ['#f6b513', '#f6b513', '#f6b513'];
  xAxisLabel = 'Develop Acquisition Plan';

  constructor(private restAPIService: RESTAPIService, private router: Router) {
    this.restAPIService.getActivities(`WorkCategory eq 'Develop Acquisition Plan' and Status eq 'Completed'`).subscribe((response: any) => {
