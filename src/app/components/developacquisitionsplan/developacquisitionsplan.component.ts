...
@Component({
  selector: 'app-developacquisitionsplan',
  templateUrl: './developacquisitionsplan.component.html'
})
export class DevelopAcquisitionsPlanComponent extends PageComponentParent {
  ...
  constructor(private router: Router, private restAPIService: RESTAPIService, public commonService: CommonService) {
    super();
    // this.restAPIService.getDevelopAcquisitionsPlanData().subscribe((response: any) => {
    ...
    //   }
    // });
    let filter = `substringof('${AppConstants.ACQ_PLAN}', WorkCategory)`;

    forkJoin([this.restAPIService.getDevelopAcquisitionsPlanData(), this.restAPIService.getFilteredTemplatesPageData(filter), this.restAPIService.getTutorialsPageData(filter)]).subscribe((forkJoinResponses: any) => {
